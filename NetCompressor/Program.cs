...
namespace NetCompressor
{
	class Program
	{
		...
		static bool flaggedSevenZipForDeletion;
		static string appToBeCompressed;
		static string outputFile;
		static List<string> dllInstructions = new List<string>();
		static string possibleMessage = "", possibleAssembly = "";
		static bool gzOr7z = true;
		static string entryPointReturnType = "System.Void";

		static ResourceManager manager = new ResourceManager("NetCompressor.Properties.Resources", Assembly.GetExecutingAssembly());
		...
		private static string GetEnd()
		{
			return manager.GetString("AppMethodEnd")
				.Replace("%ret%", entryPointReturnType == "void" ? "" : ("return (" + entryPointReturnType + ")"));
			// feels hacky, but this entire program might as well be one
		}
...
{
	[STAThread]
	static "+entryPointReturnType+@" Main(string[] args)
		...
		static void Main(string[] args)
		{
			...
			foreach (string word in args)
			{
				...
				else
				{
					...
					else if(getIcon) // a flag to read the next argument as the icon file.
					{
						iconSelected = word;
						getIcon = false;
					}
					else if (assemblyMode) // a flag to read the next argument as an assembly information file.
					{
						possibleAssembly = word;
						assemblyMode = false;
					}
					else if (messageMode) // a flag to read the next argument as a message file.
					{
						possibleMessage = word;
						messageMode = false;
					}
					else if (word.Trim().Length > 0 && word.Trim()[0] == '-')
					{
						switch (word.Trim())
						{
							case "-rip": // command flag to rip icon from previous exe (to the best of its ability).
								ripIconMode = true;
								break;
							case "-i": // command flag to grab an icon.
								getIcon = true;
								break;
							case "-a": // command flag to grab an assembly file.
								assemblyMode = true;
								break;
							case "-m": // command flag to grab a message file.
								messageMode = true;
								break;
							case "-lz0": // command flag to use lzma compression but not embed the file.
								gzOr7z = true;
								exclude7zip = true;
								break;
							case "-lz": // command flag to use lzma compression (default)
								gzOr7z = true;
								exclude7zip = false;
								break;
							case "-gz": // command flag to use gzip compression.
								gzOr7z = false;
								exclude7zip = true;
								break;
							case "-w": //command flag to compile as a window executable, and not as a console executable.
								parameters.CompilerOptions = "/target:winexe";
								break;
							case "-d": // command flag to display the generated code.
								displayCode = true;
								break;
						}
					}
			...
			string p = appToBeCompressed;
			Assembly get_entry = Assembly.Load(
				File.ReadAllBytes(
					Path.IsPathRooted(p) ? p : Directory.GetCurrentDirectory() + '\\' + p));
			entryPointReturnType = get_entry.EntryPoint.ReturnType.ToString();
			if (entryPointReturnType == "System.Void")
				entryPointReturnType = "void";
